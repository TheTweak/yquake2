...
InitGame(void)
{
	...
	g_edicts = reinterpret_cast<edict_s*>(gi.TagMalloc(game.maxentities * sizeof(g_edicts[0]), TAG_GAME));
	globals.edicts = g_edicts;
	globals.max_edicts = game.maxentities;

	/* initialize all clients for this game */
	game.maxclients = maxclients->value;
	game.clients = reinterpret_cast<gclient_t*>(gi.TagMalloc(game.maxclients * sizeof(game.clients[0]), TAG_GAME));
...
ReadField(FILE *f, field_t *field, byte *base)
{
	...
	switch (field->type)
	{
		...
		case F_LSTRING:
			...
			{
				*(char **)p = static_cast<char*>(gi.TagMalloc(32 + len, TAG_LEVEL));
...
ReadGame(char *filename)
{
	...
	g_edicts = reinterpret_cast<edict_t*>(gi.TagMalloc(game.maxentities * sizeof(g_edicts[0]), TAG_GAME));
	globals.edicts = g_edicts;

	fread(&game, sizeof(game), 1, f);
	game.clients = reinterpret_cast<gclient_t*>(gi.TagMalloc(game.maxclients * sizeof(game.clients[0]),
			TAG_GAME));
